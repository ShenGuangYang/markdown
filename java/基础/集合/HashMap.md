...
1. 容量：容量为数组的长度，即桶的个数，默认16个，最大为  ${2^{32}}$ ,当容量达到64时才可以树化
2. 装载因子：装载因子用来计算容量达到多少 时才进行扩容，默认为0.75
3. 扩容：当数量达到 ${容量 * 装载因子}$ 个时，扩容两倍
4. 树化：当容量达到64且链表长度达到8时，进行树化，当链表长度小于6时反树化 



## Node 内部类

Node是一个典型的单链表节点，其中，hash用来存储可以计算得到了hash值。
...
>
> 1. 计算key的hash值
> 2. 如果桶（table）数量为0，则直接插入
> 3. 如果key所在的桶没有元素，则直接插入
> 4. 如果key所在的桶中的第一个元素的key与待插入的key相同，说明找到了元素，转后续（9）处理
> 5. 如果第一个元素是树节点，则调用树节点的 `putTreeVal()` 寻找元素或插入树节点
> 6. 如果不是以上三种情况，则遍历桶对应的链表查找key是否存在链表中
> 7. 如果找到了对应key的元素，转后续（9）处理
> 8. 如果没有找到对应key的元素，则在链表最后插入一个新节点并判断是否需要树化
> 9. 如果找到了对应key的元素，则判断是否需要替换旧值，并直接返回旧值
> 10. 如果插入了元素，则数量 +1 并判断是否需要扩容
...
final Node<K,V>[] resize() {
    ...
    if (oldCap > 0) {
        ...
        // 如果旧容量旧扩容门槛都是0，说明还未初始化过，
        // 则初始化容量为默认容量，扩容门槛为 默认容量*默认装载因子
...
> 执行逻辑：
>
> 1. 如果使用是默认构造方法，则第一次插入元素时初始化为默认值，容量是16，扩展门槛为12
> 2. 如果使用的是非默认构造方法，则第一次插入元素时初始化容量等于扩容门槛，扩容门槛在构造方法里等于传入容量向上最近的 ${2^n}$ .
> 3. 如果旧容量大于0，则新容量等于旧容量的2倍，但不超过最大容量。新扩容门槛为旧扩容门槛的2倍
> 4. 创建一个新容量的桶
...
> 1. 寻找根节点
> 2. 从根节点开始查找
> 3. 比较hash值及key值，如果都相同，直接返回，在 `putVal()` 方法中决定是否要替换value值
...
> 1. 从链表的第一个元素开始遍历
> 2. 将第一个元素作为根节点
> 3. 其它元素一次插入到红黑树中，在做平衡
> 4. 将根节点一刀链表第一元素的位置（因为平衡的时候根节点会改变）
...
> 4. 如果第一个元素是树节点就按树的方式查找，否则按链表方式查找
...
final void removeTreeNode(HashMap<K,V> map, Node<K,V>[] tab,
    ...
    // 分割线，以上都是删除链表中的节点
    // 下面才是直接删除红黑树的节点（因为TreeNode本身即是链表节点又是树节点）
...
2. HashMap的默认初始容量是16 ，加载因子是0.75f，每次扩容变成原来的两倍
3. 当桶的数量小于64时，不会进行树化，只会扩容
4. 当桶的数量大于64，且单个桶中的数量大于8时，进行树化
5. 当单个桶中元素小于6时，进行反树化
6. HashMap查找添加元素的时间复杂度都为O(1)；
